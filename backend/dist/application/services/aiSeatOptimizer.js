...
exports.AISeatOptimizer = void 0;
class AISeatOptimizer {
    /**
     ...
     */
    static suggestBestSeating(partySize, preferredZone, tables, activeReservations) {
        // 1. Filter out tables that are busy or reserved at this time
        const busyTableIds = new Set(activeReservations
            .map(r => r.tableId)
            .filter((id) => id !== null && id !== undefined));
        const availableTables = tables.filter(t => t.status === 'AVAILABLE' && !busyTableIds.has(t.id));
...
exports.AISeatOptimizer = AISeatOptimizer;
